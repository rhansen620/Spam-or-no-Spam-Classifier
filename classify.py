...
def read_smsspam(smsspam_path):#: str) -> Iterator[Tuple[Text, Text]]:
    ...
    """
    smsspam = open(smsspam_path) 
    bm = []
    for line in smsspam:
        label = line.split().pop(0) 
        text = line.split("\t").pop(1)
        r =([label, text.rstrip("\n")])
        bm.append(r)
    return bm
    print(bm)
...
class TextToFeatures:
    def __init__(self, texts):# Iterable[Text]):
        ...
        """
        self.vectorizer = CountVectorizer(ngram_range=(1, 2), min_df=10, max_df=.8)    
...
class TextToLabels:
    def __init__(self, labels): #Iterable[Text]):
        ...
        """
        self.lab = preprocessing.LabelEncoder()
        self.lab.fit(labels)
    ...
    def index(self, label):# Text) -> int:
        ...
        """
        return self.lab.transform([label])[0]
    ...
    def __call__(self, labels): #Iterable[Text]) -> NDArray:
        ...
        """
        return self.lab.transform(labels)

class Classifier:
    def __init__(self):
        """Initalizes a logistic regression classifier.
        """
        self.logreg = LogisticRegression(fit_intercept= True,class_weight= {0:3,1:9}, random_state = 0)
    ...
    def train(self, features: NDArray, labels: NDArray) -> None:
        ...
        self.logreg.fit(features, labels, sample_weight=None)
